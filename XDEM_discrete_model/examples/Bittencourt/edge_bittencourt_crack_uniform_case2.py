...
epochs_per_state = 20000 # 5000轨迹很像了
patience = 10
lr_init = 1e-3
milestones = [4000]
eval_sep = 200
...
point_num = 300         # 域内基础网格密度
a_increment = 0.5       # 裂纹步长
...
r_J = 3.1
# 输出目录
out_dir = '../../result/bittencourt_case2'
...
class Plate(PINN2D):
    ...
    def Energy_loss(self)->torch.Tensor:
        return torch.stack([((W*H)-3*0.25**2*3.14159)*self.E_int() - self.E_ext()])
...
def run_load_control_with_growth(a_increment):
    ...
    points = [notch_root.tolist(), [-6.0, -3.5], p_tip.tolist()]
    ...
    for il, U in enumerate(U_schedule, start=1):
        ...
        for m in range(20):
            ...
            K1, K2, J = estimate_KJ(pinn, embedding, [tipx, tipy], beta_local, radius=0.5) 
            ...
            # K1, K2, J = estimate_KJ(pinn, embedding, [-5.0, 0.0], beta_local, radius=r_J) # 一个固定的圆，因为J积分容易绕道孔里面
# #  用张开位移算一下
