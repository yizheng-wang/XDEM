...
class Plate(PINN2D):
    ...
    def hard_u(self, u, x, y):
        ...
        beta1_t = torch.as_tensor(beta1, device=x.device, dtype=x.dtype)
        beta2_t = torch.as_tensor(beta2, device=x.device, dtype=x.dtype)
        c1 = torch.cos(beta1_t)
        s1 = torch.sin(beta1_t)
        c2 = torch.cos(beta2_t)
        s2 = torch.sin(beta2_t)
        # 全局->局部 的旋转 R = [[c, s],[-s, c]]
        # 右/左裂尖（全局坐标）
        ...
        # Calculate Mode I/II crack analytical solution (plane strain)
        mu = E / (2 * (1 + nu))
        kappa = 3 - 4 * nu
...
def run_for_angle(angle_deg: int):
    global beta1, beta2
    beta1 = torch.tensor(np.deg2rad(angle_deg))
    beta2 = -beta1
    a = 0.5
    b = 1.0
    h = 1.0
    ...
    # 输出目录按角度分别存放
    angle_dir = f'../../result/crack_intersect/crack_XDEM/different_beta/angle_{angle_deg:02d}'
    model_name = os.path.join(angle_dir, 'crack')
    ...
    print(f"Angle {angle_deg} deg -> Energy loss:", pinn.Energy_loss())

    # =============================
    # Field visualization (cloud maps)
    # =============================
    print(f"Generating field visualizations and embedding plot for angle {angle_deg}...")
    ...
    # u1 displacement
    plt.subplot(2, 3, 1)
    contour_u1 = plt.contourf(X_vis_np, Y_vis_np, u_pred_np, levels=level_num, cmap='RdBu_r')
    plt.colorbar(contour_u1, label='u1 displacement (mm)')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'u1 Displacement Field (β={angle_deg}°)')
    plt.axis('equal')

    # u2 displacement
    plt.subplot(2, 3, 2)
    contour_u2 = plt.contourf(X_vis_np, Y_vis_np, v_pred_np, levels=level_num, cmap='RdBu_r')
    plt.colorbar(contour_u2, label='u2 displacement (mm)')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'u2 Displacement Field (β={angle_deg}°)')
    plt.axis('equal')

    # σ11 stress
    plt.subplot(2, 3, 3)
    contour_sx = plt.contourf(X_vis_np, Y_vis_np, sx_pred_np, levels=level_num, cmap='RdBu_r')
    plt.colorbar(contour_sx, label='σ11 stress (MPa)')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'σ11 Stress Field (β={angle_deg}°)')
    plt.axis('equal')

    # σ22 stress
    plt.subplot(2, 3, 4)
    contour_sy = plt.contourf(X_vis_np, Y_vis_np, sy_pred_np, levels=level_num, cmap='RdBu_r')
    plt.colorbar(contour_sy, label='σ22 stress (MPa)')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'σ22 Stress Field (β={angle_deg}°)')
    plt.axis('equal')

    # σ12 stress
    plt.subplot(2, 3, 5)
    contour_sxy = plt.contourf(X_vis_np, Y_vis_np, sxy_pred_np, levels=level_num, cmap='RdBu_r')
    plt.colorbar(contour_sxy, label='σ12 stress (MPa)')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'σ12 Stress Field (β={angle_deg}°)')
    plt.axis('equal')

    # Von Mises stress
    plt.subplot(2, 3, 6)
    contour_vm = plt.contourf(X_vis_np, Y_vis_np, von_mises, levels=level_num, cmap='RdBu_r')
    plt.colorbar(contour_vm, label='Von Mises stress (MPa)')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'Von Mises Stress Field (β={angle_deg}°)')
    plt.axis('equal')
    ...
    # Gamma Embedding Field 
    plt.figure(figsize=(5, 4))
    contour_gamma = plt.contourf(X_vis_np, Y_vis_np, gamma_pred_np, levels=level_num, cmap='RdBu_r')
    plt.colorbar(contour_gamma, label='Embedding function')
    plt.xlabel('x')
    plt.ylabel('y')
    plt.title(f'Embedding function Field (β={angle_deg}°)')
    plt.xlim(-b, b)
    plt.ylim(-h, h)
    plt.axis('equal')
    plt.grid(True, alpha=0.3)
