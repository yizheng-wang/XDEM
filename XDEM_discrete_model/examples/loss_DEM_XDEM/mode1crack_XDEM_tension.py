...
class Plate(PINN2D):
    ...
    def hard_u(self, u, x, y):
        ...
        return (u + u_analytical_left * torch.exp(-5*self.b/self.a*r_left**self.q) + u_analytical_right * torch.exp(-5*self.b/self.a*r_right**self.q)) * x
    ...
    def hard_v(self, v, x, y):
        ...
        u_analytical_left = (self.K_I / (2 * mu)) * torch.sqrt(r_left / (2 * np.pi)) * torch.cos(theta_left/2) * (kappa - 1 + 2 * torch.sin(theta_left/2)**2)
        v_analytical_left = (self.K_I / (2 * mu)) * torch.sqrt(r_left / (2 * np.pi)) * torch.sin(theta_left/2) * (kappa + 1 - 2 * torch.cos(theta_left/2)**2)
        return (v + v_analytical_left * torch.exp(-5*self.b/self.a*r_left**self.q) + v_analytical_right * torch.exp(-5*self.b/self.a*r_right**self.q))  * (y+ self.h)/2
    ...
    def train_with_k1_monitoring(self, epochs=50000, patience=10, path='test', lr=0.02, eval_sep=100,
                                 milestones=[5000,7000,8000,9000,10000,11000,12000,13000,14000,15000,16000,17000,18000,19000,20000], crack_embedding=None, 
        ...
        for i in range(epochs):
            ...
            if self.iter % eval_sep == 0:
                ...
                self.k1_history.append((self.iter, k1_calculated_j, k1_calculated_disp, strain_energy, external_work))
                print(f'Epoch {self.iter:6d} | K_I_J = {k1_calculated_j:.3f} | K_I_Disp = {k1_calculated_disp:.3f} | K_I_NN = {self.K_I.cpu().detach().numpy():.3f} | K_I_True = {self.K1_true:.3f} | K_I_Error = {abs(k1_calculated_j - self.K1_true)/self.K1_true:.3f} | Strain_Energy = {strain_energy:.3e} | External_Work = {external_work:.3e} | Total_Loss = {current_loss:.3e} | J_Integral = {k1_calculated_j:.3e}')
    ...
    def save_k1_history(self, name):
        ...
        if hasattr(self, 'k1_history') and self.k1_history:
            import pandas as pd
            # Check if there are calculated K1 values and energy components
            if len(self.k1_history[0]) == 5:
                k1_df = pd.DataFrame(self.k1_history, columns=['epoch', 'K_I_J', 'K_I_Disp', 'Strain_Energy', 'External_Work'])
            elif len(self.k1_history[0]) == 3:
    ...
    def plot_k1_training_curve(self, name):
        ...
        if hasattr(self, 'k1_history') and self.k1_history:
            ...
            if len(self.k1_history[0]) == 5:
                # With energy data
                epochs, k1_values_J, k1_values_Disp, strain_energy, external_work = zip(*self.k1_history)
                ...
                ax1.set_ylabel('K_I')
                ax1.set_title('K_I Parameter Training Process')
                ...
                ax2.set_title('Energy Components Training Process')
                ax2.grid(True, alpha=0.3)
                ax2.legend()
                
                plt.tight_layout()
                plt.savefig(name + '_k1_training_curve.png', dpi=300, bbox_inches='tight')
                print(f"K_I training curve with energy components saved to: {name}_k1_training_curve.png")
...
b = 1.0
h = 3.0
q = 1 # 指数衰减因子 a=0.5时候，q=1效果最好，a=0.2时候，q=2
point_num = 20
total_epoch = 15000
x_crackTip = a
model_name = f'../../result/loss_compare/crack_XDEM/a_{a:.1f}/crack'
...
pinn.train_with_k1_monitoring(path=model_name, patience=10, epochs=total_epoch, lr=0.01, eval_sep=100,
