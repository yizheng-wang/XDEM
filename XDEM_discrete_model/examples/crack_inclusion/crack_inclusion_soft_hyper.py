...
decay_alpha_num = 50
# ---------------- 夹杂参数（圆形） ----------------
# 圆心 (0.25, 0.0)，半径 0.2；夹杂材料 E=210.0e2, ν=0.3
incl_cx, incl_cy = 0.0, -2.0
incl_r = 1.0
E_inclusion = 210.0e2
nu_inclusion = 0.3
# ---------------- 位移控制加载参数 ----------------
...
U_schedule = np.concatenate([
    np.linspace(0.001, 0.050, num=50, endpoint=True)
])
# ---------------- 裂纹扩展参数 ----------------
a_increment = 0.5    # 每次扩展长度
...
# 输出目录
out_dir = '../../result/crack_inclusion/displacement_control_growth/soft_hyper'
...
class Plate(PINN2D):
    ...
    def hard_v(self, v, x, y):
        ...
        return (v/30 ) * (1 - y/self.h) * (1 + y/self.h) + self.Ubar * (1 + y/self.h)/2
    ...
    # ---------------- 超弹性（Neo-Hookean）+ 圆形夹杂：空间变材料参数 ----------------
    def _material_muK(self, xy: torch.Tensor):
        # 根据位置选择 E、nu（夹杂内部/基体），并换算 mu、K
        x = xy[..., 0]
        y = xy[..., 1]
        r = torch.sqrt((x - torch.tensor(incl_cx, device=self.device, dtype=x.dtype))**2 +
                       (y - torch.tensor(incl_cy, device=self.device, dtype=y.dtype))**2)
        in_incl = (r <= torch.tensor(incl_r, device=self.device, dtype=r.dtype))

        # 剪切模量 mu 与 体积模量 K（可压缩Neo-Hookean）
        mu_m = torch.tensor(E/(2*(1+nu)), device=self.device)
        K_m  = torch.tensor(E/(3*(1-2*nu)), device=self.device)
        mu_i = torch.tensor(E_inclusion/(2*(1+nu_inclusion)), device=self.device)
        K_i  = torch.tensor(E_inclusion/(3*(1-2*nu_inclusion)), device=self.device)

        mu = torch.where(in_incl, mu_i, mu_m)
        K  = torch.where(in_incl, K_i,  K_m)
        return mu, K

    def _finite_strain_F(self, u, v, xy):
        # 形函数: F = I + grad(u)
        du_dxy = self.get_grad_wrapper(u, xy)
        dv_dxy = self.get_grad_wrapper(v, xy)
        du_dx, du_dy = du_dxy[..., 0], du_dxy[..., 1]
        dv_dx, dv_dy = dv_dxy[..., 0], dv_dxy[..., 1]
        one = torch.ones_like(du_dx)
        zero = torch.zeros_like(du_dx)
        # F = [[1+du/dx, du/dy],[dv/dx, 1+dv/dy]]
        F11 = one + du_dx
        F12 = du_dy
        F21 = dv_dx
        F22 = one + dv_dy
    ...
    def get_grad_wrapper(self, f, xy):
        # 与基类 get_grad 一致的包装，避免外部依赖
        from utils.get_grad import get_grad as _g
        return _g(f, xy)

    def _cauchy_stress_neo_hookean(self, F11, F12, F21, F22, mu, K):
        # B = F * F^T
        B11 = F11*F11 + F12*F12
        B12 = F11*F21 + F12*F22
        B22 = F21*F21 + F22*F22
        ...
        # sigma = mu/J (B - I) + (K * ln J)/J * I   ← 与你的 W 一致
        lnJ = torch.log(Jc)
        invJ = 1.0 / Jc

        I11 = torch.ones_like(B11)
        I22 = torch.ones_like(B22)

        s11 = mu*invJ*(B11 - I11) + K*lnJ*invJ
        s22 = mu*invJ*(B22 - I22) + K*lnJ*invJ
        s12 = mu*invJ*(B12)
        return s11, s22, s12, Jc, B11, B22

    def pred_stress(self, xy):
        u, v = self.pred_uv(xy)
        F11, F12, F21, F22 = self._finite_strain_F(u, v, xy)
        mu, K = self._material_muK(xy)
        sx, sy, sxy, _, _, _ = self._cauchy_stress_neo_hookean(F11, F12, F21, F22, mu, K)
        return sx, sy, sxy
    ...
    def get_energy_density(self, xy):
        # 可压缩Neo-Hookean能量（以B与J表示）：
        # W = 0.5*mu*(trace(B) - 2 - 2*ln J) + 0.5*K*(ln J)^2  (2D近似)
        u, v = self.pred_uv(xy)
        F11, F12, F21, F22 = self._finite_strain_F(u, v, xy)
        mu, K = self._material_muK(xy)
        # B, J
        ...
        J = F11*F22 - F12*F21
        Jc = torch.clamp(J, min=1e-6)
        I1 = B11 + B22  # 2D trace(B)
        lnJ = torch.log(Jc)
        W = 0.5*mu*(I1 - 2.0 - 2.0*lnJ) + 0.5*K*(lnJ*lnJ)
        return W
...
def run_displacement_loading_with_growth():
    ...
    p0 = np.array([-2.0, 0.0], dtype=float)  # 起点（左端）
    ...
    for il, Ubar in enumerate(U_schedule, start=1):
        ...
        while True:
            ...
            # === 每一步裂纹扩展都绘制并保存云图（2×3与单图） ===
            try:
                img_path_prop = os.path.join(step_dir, f'prop{prop_cnt}_fields.png')
                plot_comprehensive_fields(pinn, embedding, out_path=img_path_prop, test_num=300)
            except Exception as e:
                print(f"[warn] failed to plot fields for prop{prop_cnt}: {e}")
...
def plot_comprehensive_fields(pinn, embedding, out_path=None, test_num=300):
    ...
    if out_path is not None:
        ...
        # === 保存所有场数据到 .npz 文件（便于后处理） ===
        npz_path = f'{base_path}_data.npz'
        np.savez(npz_path, 
                 X=Xn, Y=Yn,
                 u1=un, u2=vn,
                 sigma11=sxn, sigma22=syn, sigma12=sxyn,
                 von_mises=von_mises_n,
                 gamma=gn)
        print(f"[plot_comprehensive_fields] field data saved -> {npz_path}")
