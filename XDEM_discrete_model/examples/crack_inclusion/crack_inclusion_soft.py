...
# 输出目录
out_dir = '../../result/crack_inclusion/displacement_control_growth/soft'
...
class Plate(PINN2D):
    ...
    def hard_u(self, u, x, y):
        ...
        return (u/30 + u_analytical_right_global * torch.exp(-5*self.b/self.a*r_right**self.q)) * (1 + y/self.h)
    ...
    def hard_v(self, v, x, y):
        ...
        return (v/30 + v_analytical_right_global * torch.exp(-5*self.b/self.a*r_right**self.q)) * (1 - y/self.h) * (1 + y/self.h) + self.Ubar * (1 + y/self.h)/2
    ...
    # ---------------- 异质材料支持（圆形夹杂区） ----------------
    def _material_tensors(self, xy: torch.Tensor):
        # 根据位置选择 E、nu（夹杂内部/基体）
        x = xy[..., 0]
        y = xy[..., 1]
        ...
        if self.plane_strain:
            d11_m = torch.tensor(E*(1-nu)/((1+nu)*(1-2*nu)), device=self.device)
            d12_m = torch.tensor(E*nu/((1+nu)*(1-2*nu)), device=self.device)
            G_m   = torch.tensor(E/(2*(1+nu)), device=self.device)

            d11_i = torch.tensor(E_inclusion*(1-nu_inclusion)/((1+nu_inclusion)*(1-2*nu_inclusion)), device=self.device)
            d12_i = torch.tensor(E_inclusion*nu_inclusion/((1+nu_inclusion)*(1-2*nu_inclusion)), device=self.device)
            G_i   = torch.tensor(E_inclusion/(2*(1+nu_inclusion)), device=self.device)
        else:
            d11_m = torch.tensor(E/(1-nu*nu), device=self.device)
            d12_m = torch.tensor(E*nu/(1-nu*nu), device=self.device)
            G_m   = torch.tensor(E/(2*(1+nu)), device=self.device)

            d11_i = torch.tensor(E_inclusion/(1-nu_inclusion*nu_inclusion), device=self.device)
            d12_i = torch.tensor(E_inclusion*nu_inclusion/(1-nu_inclusion*nu_inclusion), device=self.device)
            G_i   = torch.tensor(E_inclusion/(2*(1+nu_inclusion)), device=self.device)

        d11 = torch.where(in_incl, d11_i, d11_m)
        d12 = torch.where(in_incl, d12_i, d12_m)
        G   = torch.where(in_incl, G_i,   G_m)
        return d11, d12, G

    def pred_stress(self, xy):
        u, v = self.pred_uv(xy)
        eXX, eYY, eXY = self.compute_Strain(u, v, xy)
        d11, d12, G = self._material_tensors(xy)
        sx = d11 * eXX + d12 * eYY
        sy = d12 * eXX + d11 * eYY
        sxy = G * eXY
        return sx, sy, sxy
...
def run_displacement_loading_with_growth():
    ...
    for il, Ubar in enumerate(U_schedule, start=1):
        ...
        while True:
            ...
            # 评估 K 与 J
            K1, K2, J = estimate_KJ(pinn, embedding, [tipx, tipy], beta_local, radius=a0)
