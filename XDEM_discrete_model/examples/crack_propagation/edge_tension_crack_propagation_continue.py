...
U_schedule = np.concatenate([
    np.linspace(0.001, 0.004, num=4, endpoint=True), np.linspace(0.0042, 0.006, num=10, endpoint=True)
])
# ---------------- 裂纹扩展参数 ----------------
...
# 输出目录
out_dir = '../../result/crack_propagation/displacement_control_growth'
...
class Plate(PINN2D):
    def __init__(self, model: nn.Module, a, b, h, plane_strain=True):
        ...
        self.x_crackTip = 0.0
        self.y_crackTip = 0.0
        K1_esti = 0.001*E * np.sqrt(np.pi * a)  * np.cos(self.beta)**2
        K2_esti = 0.001*E * np.sqrt(np.pi * a)  * np.sin(self.beta) * np.cos(self.beta)
    ...
    def hard_u(self, u, x, y):
        ...
        return (u/1000 + u_analytical_right_global * torch.exp(-5*self.b/self.a*r_right**self.q)) * (1 + y/self.h)/2
    ...
    def hard_v(self, v, x, y):
        ...
        return (v/1000 + v_analytical_right_global * torch.exp(-5*self.b/self.a*r_right**self.q))  * (1-y/self.h) * (1 + y/self.h) + self.Ubar * (1 + y/self.h)/2
...
def make_net_with_embedding(crack_points):
    ...
    try:
        embedding = Embedding.multiLineCrackEmbedding(crack_points, tip='right', decay_alpha=decay_alpha_num)
    except Exception:
        # 若没有 multiLineCrackEmbedding，也可退化为末段 LineCrackEmbedding
        p0, p1 = crack_points[-2], crack_points[-1]
        embedding = LineCrackEmbedding(p0, p1, tip='right', decay_alpha=decay_alpha_num)
...
# ======== 从某步Ubar(如0.006)继续往上加载并训练/扩展 ========
import re, os
from glob import glob

def _find_step_dir_by_U(out_dir, U_target, tol=5e-7):
    want = f"{U_target:.6f}"
    cand = sorted(glob(os.path.join(out_dir, f"step_*_Ubar_{want}")))
    if cand:
        return cand[-1]
    # 容错遍历
    for d in sorted(glob(os.path.join(out_dir, "step_*_Ubar_*"))):
        m = re.search(r"Ubar_([0-9.]+)$", d)
        if m and abs(float(m.group(1)) - U_target) <= tol:
            return d
    return None

def _latest_prop_tag(step_dir):
    """返回 (基名, n)。若只存在 prop0 则 n=0"""
    props = sorted(glob(os.path.join(step_dir, "prop*.pth")))
    if not props:
        base = os.path.join(step_dir, "prop0")
        return base, 0
    last = props[-1]
    base = last[:-4]
    m = re.search(r"prop(\d+)\.pth$", last)
    n = int(m.group(1)) if m else 0
    return base, n

def _next_step_index(out_dir):
    """找现有最大 step_XX 的 XX，返回下一个索引（从 1 开始）"""
    steps = sorted(glob(os.path.join(out_dir, "step_*_Ubar_*")))
    max_id = 0
    for d in steps:
        bn = os.path.basename(d)
        m = re.search(r"step_(\d+)_", bn)
        if m:
            max_id = max(max_id, int(m.group(1)))
    return max_id + 1

def _sync_tip_from_points(pinn, points):
    last_seg = LineSegement(points[-2], points[-1])
    pinn.x_crackTip = float(points[-1][0])
    pinn.y_crackTip = float(points[-1][1])
    pinn.beta = float(last_seg.tangent_theta.numpy())

def continue_from_U(
    U_start=0.006,
    U_next_schedule=None,   # 例如 np.linspace(0.0062, 0.008, 10)
    ...
    assert U_next_schedule is not None and len(U_next_schedule) > 0, "请传入后续的 U_next_schedule（数组或列表）"

    # 1) 找到 U_start 对应目录 & 读取几何/权重
    step_dir0 = _find_step_dir_by_U(out_dir, U_start)
    if step_dir0 is None:
        raise FileNotFoundError(f"未找到已有的 Ubar={U_start:.6f} 结果目录，请确认 {out_dir} 下是否存在。")

    # step id（用于 LoRA 命名读取）
    base_name0 = os.path.basename(step_dir0)  # e.g., "step_14_Ubar_0.006000"
    m0 = re.search(r"step_(\d+)_", base_name0)
    start_step_id = m0.group(1) if m0 else "00"

    # points
    pts_path = os.path.join(step_dir0, "points.pt")
    if not os.path.exists(pts_path):
        raise FileNotFoundError(f"缺少 {pts_path}，无法恢复几何。")
    points = torch.load(pts_path)
    if isinstance(points, torch.Tensor):
        points = points.tolist()

    # 2) 重建网络/embedding/pinn，并加载 0.006 的最新 checkpoint + LoRA
    pinn, embedding = init_pinn(points)
    pinn.Ubar = torch.tensor(float(U_start), device=pinn.device)
    _sync_tip_from_points(pinn, points)
    pinn.model.set_extend_axis(embedding)

    last_base, last_prop_n = _latest_prop_tag(step_dir0)
    if os.path.exists(last_base + ".pth"):
        print(f"[continue] load checkpoint at U={U_start:.6f}: {last_base}.pth")
        pinn.load(path=last_base)
    else:
        print(f"[continue][warn] 未找到 {last_base}.pth，将以当前权重继续。")

    # 优先加载该步的 LoRA（propN 版本），否则降级为 lora_stepXX
    lora_prop = os.path.join(out_dir, f"lora_step{start_step_id}_prop{last_prop_n:02d}.pth")
    lora_step = os.path.join(out_dir, f"lora_step{start_step_id}.pth")
    if os.path.exists(lora_prop):
        print(f"[continue] load LoRA: {lora_prop}")
        load_lora(pinn.model, lora_prop)
    elif os.path.exists(lora_step):
        print(f"[continue] load LoRA: {lora_step}")
        load_lora(pinn.model, lora_step)
    else:
        print("[continue][warn] 未找到 LoRA 适配器，直接续跑。")
    ...
    # 3) 继续往上：为每个新 Ubar 建新 step 目录，完整执行“propagation 循环”
    next_step_id_int = _next_step_index(out_dir)
    for Ubar in U_next_schedule:
        step_id_str = f"{next_step_id_int:02d}"
        step_dir = os.path.join(out_dir, f"step_{step_id_str}_Ubar_{float(Ubar):.6f}")
        os.makedirs(step_dir, exist_ok=True)

        print(f"\n====== Continue to load step {step_id_str}: Ubar = {float(Ubar):.6f} mm ======")
        # 热启动 LoRA：从上一载荷步（刚完成的）拿
        prev_lora = os.path.join(out_dir, f"lora_step{int(step_id_str)-1:02d}.pth")
        if os.path.exists(prev_lora):
            print(f"[LoRA] warm-start from {prev_lora}")
            load_lora(pinn.model, prev_lora)

        # 固定当前几何开始本载荷步 prop0 训练
        pinn.Ubar = torch.tensor(float(Ubar), device=pinn.device)
        pinn.model.set_extend_axis(embedding)
        pinn.set_meshgrid_inner_points(-b, b, point_num, -h, h, point_num)
        save_tag0 = os.path.join(step_dir, "prop0")
        scheduler = set_optimizer(pinn, lr)
        pinn.train(path=save_tag0, epochs=epochs_per_state, patience=patience, lr=lr,
                   eval_sep=eval_sep, milestones=milestones, tol_early_stop=tol_early_stop)
        ...
        while True:
            ...
            print(f"[step {step_id_str} | prop {prop_cnt}]  K_I={K1:.4f}, K_II={K2:.4f},  J={J:.6f}  (Gc={Gc:.6f})")
            print(f"  [debug] current NN K_I={pinn.K_I.detach().cpu().numpy():.3f}, K_II={pinn.K_II.detach().cpu().numpy():.3f}")

            if J >= J_abs_cutoff:
                print(f"  [stop] J={J:.6f} >= {J_abs_cutoff}, 停止所有后续。")
                try:
                ...
                try:
                    ...
                    # 追加保存 L-D
                    save_load_displacement(out_dir, [(float(Ubar), float(Fy))], fname='load_displacement_append')
                except Exception:
                    pass
                torch.save(points, os.path.join(step_dir, 'points.pt'))
                return  # 全流程提前结束

            if J <= Gc or prop_cnt >= max_propagations_per_load:
                if prop_cnt >= max_propagations_per_load:
                    print(f"  Reached max propagation iterations ({max_propagations_per_load}) at step {step_id_str}.")
                print(f"  Stable at step {step_id_str}: J={J:.6f} <= Gc={Gc:.6f}")
                # 保存 LoRA
                save_lora(pinn.model, os.path.join(out_dir, f"lora_step{step_id_str}.pth"))
                # 可视化与合力
                img_path = os.path.join(step_dir, 'stable_fields.png')
                plot_comprehensive_fields(pinn, embedding, out_path=img_path, test_num=300)
                Fy = compute_top_reaction(pinn, nx=600)
                ...
                try:
                    # 合并/追加入原有 L-D 数据
                    exist = os.path.join(out_dir, 'load_displacement.npy')
                    if os.path.exists(exist):
                        old = np.load(exist)
                        new = np.vstack([old, [float(Ubar), float(Fy)]])
                    else:
                        new = np.array([[float(Ubar), float(Fy)]], dtype=float)
                    np.save(exist, new)
                except Exception:
                    pass
                break  # 进入下一个 Ubar
            ...
            pinn.x_crackTip = new_tip[0]; pinn.y_crackTip = new_tip[1]; pinn.beta = new_theta

            points.append(new_tip)
            try:
                embedding = Embedding.multiLineCrackEmbedding(points, tip='right', b=b, decay_alpha=decay_alpha_num)
            except Exception:
                embedding = LineCrackEmbedding(points[-2], points[-1], tip='right', decay_alpha=decay_alpha_num)
            pinn.model.set_extend_axis(embedding)

            prop_cnt += 1
            save_tag = os.path.join(step_dir, f'prop{prop_cnt}')
            scheduler = set_optimizer(pinn, lr)
            pinn.set_meshgrid_trapz_Tip_Dense(-0.9999*b, b, -0.9999*h, 0.9999*h,
                                              points[-1][0], points[-1][1],
                                              120, 120, 130, 130, x_inteval=0.25, y_inteval=0.25)
            pinn.train(path=save_tag, epochs=epochs_per_state, patience=patience, lr=lr,
                       eval_sep=eval_sep, tol_early_stop=tol_early_stop)
            if os.path.exists(save_tag + ".pth"):
                pinn.load(path=save_tag)
            # 每次扩展后也存一份 LoRA
            save_lora(pinn.model, os.path.join(out_dir, f"lora_step{step_id_str}_prop{prop_cnt:02d}.pth"))

        # 保存该步的几何
        torch.save(points, os.path.join(step_dir, 'points.pt'))
        next_step_id_int += 1

    print("\n=== 续跑完成（从 U={:.6f} 起继续到指定序列）===".format(U_start))
...
def run_displacement_loading_with_growth():
    ...
    for il, Ubar in enumerate(U_schedule, start=1):
        ...
        if il > 1:
            ...
            prev_lora = os.path.join(out_dir, f"lora_step01.pth") # 用第一步骤的神经网络参数，可能后面会出现失败
            if os.path.exists(prev_lora):
        ...
        embedding = Embedding.multiLineCrackEmbedding(points,tip = 'right', decay_alpha=decay_alpha_num)
...
if __name__ == "__main__":
    # 自定义后续载荷序列（示例：从 0.0062 均匀加到 0.008）
    U_more = np.array([0.0062, 0.0064, 0.0065])
    continue_from_U(
        U_start=0.006,
        U_next_schedule=U_more,
        epochs_per_state=epochs_per_state,   # 直接复用你前面的变量
        lr=lr_init,
        patience=patience,
        eval_sep=eval_sep,
        milestones=milestones,
        tol_early_stop=tol_early_stop,
    )
