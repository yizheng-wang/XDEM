...
# 输出目录
out_dir = '../../result/crack_kinking_origin/displacement_control_growth'
...
def continue_from_U(
    ...
    lora_prop = os.path.join(out_dir, f"lora_step01.pth") # 用第一步骤的神经网络参数，用后面会出现失败 os.path.join(out_dir, f"lora_step{start_step_id}_prop{last_prop_n:02d}.pth")
    ...
    for Ubar in U_next_schedule:
        ...
        prev_lora = os.path.join(out_dir, f"lora_step01.pth") # 第一步骤的神经网络参数， os.path.join(out_dir, f"lora_step{int(step_id_str)-1:02d}.pth")
...
if __name__ == "__main__":
    ...
    U_more = np.linspace(0.0108, 0.0150, num=43, endpoint=True)
    continue_from_U(
        U_start=0.010700,
