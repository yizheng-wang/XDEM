...
"""Top-level package for pytorchtools."""
...
class MultiEarlyStopping(EarlyStopping):
    def __init__(self, paths, patience=7, verbose=False, delta=0, path='checkpoint.pt', trace_func=print):
        super().__init__(patience, verbose, delta, path, trace_func)
        self.paths = paths
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        if self.verbose:
            self.trace_func(f'Validation loss decreased ({self.val_loss_min:.6f} --> {val_loss:.6f}).  Saving model ...')
        for path, sub_model in zip(self.paths,model):
