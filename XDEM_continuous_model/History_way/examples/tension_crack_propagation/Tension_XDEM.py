...
if __name__ == "__main__":
    ...
    foldername =  "../../results/Tension_2D/"
    
    createFolder(foldername)

    figHeight = 5
    figWidth = 5
    nSteps = 11   # Total number of steps (4 steps with 0.001 + 60 steps with 0.0001)
    ...
    modelNN_U_lora = KANLoRA([2, 5,5,5, 2], base_activation=torch.nn.SiLU, grid_size=15, grid_range=[0,  1.0], spline_order=3,\
    ...
    for iStep in range(nSteps):
        ...
        if iStep <= 5:
            ...
            hist_deal = 'update'
        if iStep >= 6 and iStep <=26:
            deltaV = 0.0005  # Remaining 60 steps with smaller increment
            hist_deal = 'update'
        if iStep > 26: 
